...
sudo -u "$RAGBASE_USER" "$PYTHON_VERSION" -m venv "$RAGBASE_HOME/venv"
sudo -u "$RAGBASE_USER" "$RAGBASE_HOME/venv/bin/pip" install --upgrade pip -q
log "Virtual environment created"

section "Python Dependencies"
# Copy requirements to app directory
cp requirements.txt "$RAGBASE_HOME/requirements.txt"
chown "$RAGBASE_USER:$RAGBASE_USER" "$RAGBASE_HOME/requirements.txt"
...
sudo -u "$RAGBASE_USER" "$RAGBASE_HOME/venv/bin/pip" install \
    -r "$RAGBASE_HOME/requirements.txt" -q
...
from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())")
    ...
    warn "Generated MCP token: $MCP_TOKEN"
    warn "Add to Claude Desktop config as the Bearer token."
    warn "Fill in ANTHROPIC_API_KEY, DROPBOX_ACCESS_TOKEN, DOMAIN, and BACKUP_PASSPHRASE in $RAGBASE_HOME/.env"
else
...
# DOMAIN placeholder will be substituted from .env when Caddy starts
# We use an environment variable in the Caddyfile
systemctl enable caddy
log "Caddyfile installed"

section "Systemd Services"
cp systemd/ragbase-api.service /etc/systemd/system/
cp systemd/ragbase-mcp.service /etc/systemd/system/
...
echo -e "       ANTHROPIC_API_KEY   — for auto-tagging"
echo -e "       DROPBOX_ACCESS_TOKEN — for backups"
echo -e "       DOMAIN               — your domain (e.g. ragbase.yourdomain.com)"
echo -e "       BACKUP_PASSPHRASE    — strong passphrase for backup encryption"
echo ""
echo -e "  2. Install Dropbox on the droplet:"
echo -e "       cd ~ && wget -O dropbox.py 'https://www.dropbox.com/download?dl=packages/dropbox.py'"
echo -e "       python3 dropbox.py start"
echo ""
echo -e "  3. Point your domain DNS → this droplet IP, then start services:"
echo -e "       ${BLUE}systemctl start ragbase-api ragbase-mcp caddy${NC}"
echo ""
echo -e "  4. Add to Claude Desktop config (~/.claude/claude_desktop_config.json):"
echo -e "       {"
echo -e "         \"mcpServers\": {"
echo -e "           \"ragbase\": {"
...
echo -e "       }"
echo ""
echo -e "  5. Drop a PDF in your Dropbox folder — first ingest will run within 5 minutes."
echo ""
